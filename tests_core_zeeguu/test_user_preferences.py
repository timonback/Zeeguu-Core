...
class UserPreferenceTest(ModelTestMixIn, TestCase):
    ...
    def test_preferred_word(self):
        starred_words_count_before = len(self.user.starred_words)

        hauen = UserWord.find("hauen", self.de)
        self.user.starred_words.append(hauen)
        self.session.commit()

        starred_words_count_after = len(self.user.starred_words)
    ...
    def test_user_word(self):
        assert self.user.user_words() == list(map((lambda x: x.origin.word), self.user.all_bookmarks()))